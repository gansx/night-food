...
export default async function HomePage() {
  ...
  const [{ data: settings }, { data: myOrders }, { data: myTasks }, { data: featuredItems }] =
    householdId && viewer
      ? await (async () => {
        ...
        <div className="shell-head">
          <div style={{ maxWidth: 760 }}>
            ...
            <div style={{ marginTop: 22, display: "flex", gap: 12, flexWrap: "wrap" }}>
              <Link className="primary-button" href={viewer ? "/order" : "/login"}>
                {viewer ? "去点餐" : "登录进入"}
              </Link>
              <Link className="ghost-button" href={viewer ? "/me" : "/family"}>
                {viewer ? "查看我的" : "加入家庭"}
              </Link>
            </div>
          </div>
