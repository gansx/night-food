...
export function RegisterCard() {
  ...
  const [message, setMessage] = useState("注册后会自动登录，然后你可以创建家庭或输入家庭邀请码。");
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage("注册成功，正在进入家庭引导...");
      navigateToAppPath("/family");
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="glass-panel" data-testid="web-register-form" style={{ padding: 20 }}>
      ...
      <div style={{ marginTop: 14, color: "var(--text-muted)", fontSize: 14 }}>
        已经有账号？{" "}
        <Link href="/login" style={{ color: "var(--brand-dark)", fontWeight: 700 }}>
          去登录
        </Link>
      </div>
    </form>
